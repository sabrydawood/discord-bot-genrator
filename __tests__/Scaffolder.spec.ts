...
describe("Scaffolder", () => {
  ...
  const genratorPath = `${fakeDir}\\botGenrator.json`;
  const config = getGenratorConfig();
  ...
  describe("createStructure", () => {
    ...
    it("should call createStructure and call createCommand", async () => {
      ...
      jest.spyOn(path, "join").mockReturnValueOnce(genratorPath);
      await scaffolder.createStructure("command");
      expect(fileSystem.getCurrentDir).toHaveBeenCalled();
      expect(path.join).toHaveBeenCalledWith(fakeDir, "botGenrator.json");
      expect(path.join).toReturnWith(genratorPath);
      expect(fileSystem.findFile).toHaveBeenCalledWith(genratorPath);
      expect(scaffolder.createCommand).toHaveBeenCalledWith(genratorPath);
    });
    it("should call createStructure and call createEvent", async () => {
      ...
      expect(path.join).toReturnWith(genratorPath);
      expect(fileSystem.findFile).toHaveBeenCalledWith(genratorPath);
      expect(scaffolder.createEvent).toHaveBeenCalledWith(genratorPath);
    });
  });
  ...
  describe("createEvent", () => {
    ...
    it("should create an event when the directory exists", async () => {
      ...
      jest.spyOn(fileSystem, "exists").mockResolvedValue(true);
      await scaffolder.createEvent(genratorPath);
      expect(fileSystem.getCurrentDir).toHaveBeenCalledTimes(1);
