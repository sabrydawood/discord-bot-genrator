...
export function getEnvTemplate(token: string, prefix: string, dbUrl: string) {
 return `DISCORD_BOT_TOKEN=${token}\nDISCORD_BOT_PREFIX=${prefix}\nMONGO_CONNECTION=${dbUrl}`
}
...
const config = require('../botGenrator.json');
...
(async () => {
await database.login(config.mongo_uri)
...
import config from '../botGenrator.json';
...
module.exports = class MessageEvent extends BaseEvent {
  ...
  async run(client, message) {
    ...
    if (message.content.startsWith(prefix)) {
      ...
      if (command) {
        command.run(client, message, cmdArgs, settings);
      }
    }
  }
}`
...
export default class TestCommand extends BaseCommand {
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>, settings: Array<string>) {
    message.channel.send('Test command works');
  }
}`
}
...
export function getDatabaseTS() {
 return `
	import { connect } from "mongoose"
export class MongoDb {
  async login(dbUrl : string) {
     console.log("Connecting to MongoDb...");
