#!/usr/bin/env node
import { Action, checkStructType, CLIArguments, StructureType } from "./utils/index"
...
export async function main(scaffolder: Scaffolder, prompter: Prompter) {
 ...
 } else if (args.length === 2) return handleChoice(scaffolder, ...(<CLIArguments>args))
}

export function handleChoice(scaffolder: Scaffolder, action: Action, data: string) {
